...
class TestIntegration:
    ...
    def test_complete_job_workflow(self, temp_script):
        ...
        if not submit_result.get("isError") and "job_id" in submit_result:
            ...
            stdout_result = get_job_output_handler(job_id, "stdout")
            assert isinstance(stdout_result, dict)
            
            stderr_result = get_job_output_handler(job_id, "stderr")
    ...
    def test_array_job_workflow(self, array_script):
        ...
        # Submit array job
        submit_result = submit_array_job_handler(
            array_script,
            array_range="1-3",
            cores=1,
            memory="1GB",
        ...
        if not submit_result.get("isError") and "array_job_id" in submit_result:
            ...
            status_result = check_job_status_handler(array_job_id)
            assert isinstance(status_result, dict)
            
            # Get array job details
            details_result = get_job_details_handler(array_job_id)
            assert isinstance(details_result, dict)
    ...
    def test_job_monitoring_workflow(self, temp_script):
        ...
        # List current jobs
        job_list = list_slurm_jobs_handler()
        ...
        if not submit_result.get("isError") and "job_id" in submit_result:
            ...
            updated_queue = get_queue_info_handler()
            assert isinstance(updated_queue, dict)
            
            # Clean up
            cancel_slurm_job_handler(job_id)

    def test_error_propagation(self):
        """Test that errors propagate correctly through the system."""
        # Test with invalid script
        result = submit_slurm_job_handler("nonexistent_script.sh", 1)
        assert isinstance(result, dict)
        assert result.get("isError") or "error" in result

        # Test with invalid job ID
        result = check_job_status_handler("invalid_job_id_12345")
        assert isinstance(result, dict)
        # Should handle gracefully - either error flag or meaningful response

        # Test with invalid parameters
        result = submit_slurm_job_handler("test.sh", 0)  # Invalid cores
        assert isinstance(result, dict)
        assert result.get("isError") or "error" in result
    ...
    def test_concurrent_operations(self, temp_script):
        ...
        if job_ids:
            status_results = [check_job_status_handler(job_id) for job_id in job_ids]
            
            for result in status_results:
                assert isinstance(result, dict)
            
            # Clean up jobs
            cancel_results = [cancel_slurm_job_handler(job_id) for job_id in job_ids]
...
"""
    ...
    def test_filtered_operations(self):
        ...
        all_jobs = list_slurm_jobs_handler()
        assert isinstance(all_jobs, dict)
        
        user_jobs = list_slurm_jobs_handler(user="testuser")
        assert isinstance(user_jobs, dict)
        
        running_jobs = list_slurm_jobs_handler(state="RUNNING")
        assert isinstance(running_jobs, dict)
        
        # Test queue info with partition filter
        all_queue = get_queue_info_handler()
        assert isinstance(all_queue, dict)
        
        partition_queue = get_queue_info_handler(partition="compute")
    ...
    def test_parameter_combinations(self, temp_script):
        ...
        for params in parameter_sets:
            result = submit_slurm_job_handler(temp_script, **params)
            assert isinstance(result, dict)
    ...
    def test_robustness_under_load(self, temp_script):
        ...
        # Job submissions
        for i in range(5):
            result = submit_slurm_job_handler(temp_script, cores=1, job_name=f"load_test_{i}")
            results.append(result)
        
        # Information gathering
        results.append(get_slurm_info_handler())
        results.append(get_node_info_handler())
        results.append(get_queue_info_handler())
        results.append(list_slurm_jobs_handler())
    ...
    def test_data_consistency_across_calls(self, temp_script):
        ...
        # Submit a job
        submit_result = submit_slurm_job_handler(temp_script, cores=1, job_name="consistency_test")
        
        if not submit_result.get("isError") and "job_id" in submit_result:
            job_id = submit_result["job_id"]
            
            # Get job information through different methods
            status1 = check_job_status_handler(job_id)
            details1 = get_job_details_handler(job_id)
            
            # Small delay
            time.sleep(0.5)
            
            # Get information again
            status2 = check_job_status_handler(job_id)
            details2 = get_job_details_handler(job_id)
